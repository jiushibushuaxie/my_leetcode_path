...
//cmp需要定义为全局函数
bool cmp(const vector<int>& a,const vector<int>& b){   //二维vector排序
        return a[0]<b[0];
}
...
public:
    int findMinArrowShots(vector<vector<int>>& points) {
        ...
        for(int i=1; i<points.size();++i){
            if(points[i][0]<=shoot_end){            //可以击穿另一个气球的条件，很关键，看不懂就画画图
                shoot_begin = points[i][0]>shoot_begin?points[i][0]:shoot_begin;
